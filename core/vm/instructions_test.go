...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, evmInterpreter, &ScopeContext{nil, stack, nil}, false)
		if len(stack.data) != 1 {
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.push(y)
		stack.push(x)
		opAddmod(&pc, evmInterpreter, &ScopeContext{nil, stack, nil}, false)
		actual := stack.pop()
		if actual.Cmp(expected) != 0 {
...
func TestWriteExpectedValues(t *testing.T) {
	...
	getResult := func(args []*twoOperandParams, opFn executionFunc) []TwoOperandTestcase {
		...
		for i, param := range args {
			...
			stack.push(x)
			stack.push(y)
			opFn(&pc, interpreter, &ScopeContext{nil, stack, nil}, false)
			actual := stack.pop()
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, evmInterpreter, scope, false)
		stack.pop()
	}
	bench.StopTimer()
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evmInterpreter, &ScopeContext{mem, stack, nil}, false)
...
func TestOpTstore(t *testing.T) {
	...
	stack.push(new(uint256.Int))
	opTstore(&pc, evmInterpreter, &scopeContext, false)
	...
	// push the location to the stack
	stack.push(new(uint256.Int))
	opTload(&pc, evmInterpreter, &scopeContext, false)
...
func BenchmarkOpKeccak256(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		stack.push(start)
		opKeccak256(&pc, evmInterpreter, &ScopeContext{mem, stack, nil}, false)
	}
}
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opRandom(&pc, evmInterpreter, &ScopeContext{nil, stack, nil}, false)
		if len(stack.data) != 1 {
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opBlobHash(&pc, evmInterpreter, &ScopeContext{nil, stack, nil}, false)
		if len(stack.data) != 1 {
			t.Errorf("Expected one item on stack after %v, got %d: ", tt.name, len(stack.data))
		}
		actual := stack.pop()
...
func TestOpMCopy(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		// Do the copy
		opMcopy(&pc, evmInterpreter, &ScopeContext{mem, stack, nil}, false)
